...
const LinkRow: React.FC<LinkRowProps> = ({ label, sub, value, onClick, danger }) => (
  <button
        ...
        <div className="sett-container">
          ...
          <div className="sett-profile-card">
            <div className="sett-avatar-wrap">
              {user?.picture || (user as any)?.avatar_url ? (
                <img
                  src={user?.picture || (user as any)?.avatar_url}
                  alt={user?.full_name}
            ...
            <div className="sett-profile-info">
              <p className="sett-profile-name">{user?.full_name || (user as any)?.name || '—'}</p>
              <p className="sett-profile-email">{user?.email || '—'}</p>
              <span className="sett-profile-badge">
                {(user as any)?.email_verified ? '✓ Verified Account' : 'Unverified'}
              </span>
            </div>
          ...
          <div className="sett-grid">
            ...
            <Section icon={<FaUser />} title="Account">
              ...
              <LinkRow
                label="Phone Number"
                sub={(user as any)?.phone_verified ? 'Verified' : 'Not added yet'}
                value={(user as any)?.phone_verified ? 'Verified' : null}
                onClick={() => {}}
              />
              <LinkRow
                label="Google Account"
                sub={
                  (user as any)?.iss?.includes('google') ? 'Connected via Google' : 'Not connected'
                }
                value={(user as any)?.iss?.includes('google') ? 'Connected' : null}
                onClick={() => {}}
              />
            </Section>
