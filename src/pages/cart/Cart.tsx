...
import './cart.css';
import { useCartItems, useUpdateCartQuantity, useRemoveFromCart } from '../../tanstack/cart';
...
const Cart: React.FC = () => {
  ...
  const updateItemQuantity = (item: CartItem, action: 'increase' | 'decrease') => {
    if (!user) return;
  ...
  const handleRemoveFromCart = (item: CartItem) => {
    if (!user) return;
    removeCartMutation.mutate({ userId: user.id, productId: String(item.product_id) });
  };
