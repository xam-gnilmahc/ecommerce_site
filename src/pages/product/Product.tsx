...
import { usePlaceOrderSingle } from '../../tanstack/orders';
import { trackProductPreview, trackAddToCart, trackPurchase } from '../../tanstack/tracking';
import GooglePayButton from '@google-pay/button-react';
import { buildPaymentRequest, getUpdatedPaymentData } from '../../components/product/GooglePlay';
import { processGooglePay } from '../../services/googlePayService';
...
interface FullProduct extends ProductType {
...
const Product: React.FC = () => {
  ...
  // ── Inventory state
  const [stockQty, setStockQty] = useState<number | null>(null); // null = loading
  ...
  const addProduct = async (product: FullProduct) => {
    ...
    await trackAddToCart(user?.id, product as any);
  };

  async function handleLoadPaymentData(paymentData: any) {
    ...
    try {
      ...
      if (gpayShippingId) {
        const opt = shippingOptions.find((o: any) => o.id === gpayShippingId);
        if (opt) {
  ...
  useEffect(() => {
    if (product && user) {
      trackProductPreview(user.id, product as any);
  ...
  const sizesFullName: Record<string, string> = {
    XS: 'Extra Small',
    S: 'Small',
    M: 'Medium',
    L: 'Large',
        ...
        {paymentRequest && user && stockQty !== 0 && (
          <GooglePayButton
            ...
            onPaymentDataChanged={(paymentData: any) =>
              getUpdatedPaymentData(paymentRequest, paymentData) as any
            }
          />
        )}
