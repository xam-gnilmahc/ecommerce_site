...
const getRaffleImageUrl = (path: string | null): string | null => {
  if (!path) return null;
  return `${SUPABASE_STORAGE_URL}raffle/${path}`;
};

const getRaffleStatus = (raffle: Raffle): 'upcoming' | 'live' | 'closed' => {
  const now = new Date();
...
/* ─────────────────────────────────────────────
   UPCOMING OVERLAY
───────────────────────────────────────────── */
interface UpcomingOverlayProps {
  startDate: string;
}

const UpcomingOverlay: React.FC<UpcomingOverlayProps> = ({ startDate }) => {
  const { d, h, m, s } = useCountdown(startDate);
  return (
    <div className="raffle-upcoming-overlay">
      <div className="raffle-upcoming-content">
        <span className="raffle-upcoming-label">Opens in</span>
        <div className="raffle-countdown">
          {[
            ...
            <div key={unit} className="raffle-countdown-block">
              <span className="raffle-countdown-num">{String(val).padStart(2, '0')}</span>
              <span className="raffle-countdown-unit">{unit}</span>
            </div>
          ))}
        </div>
      </div>
...
interface RaffleCardProps {
  raffle: Raffle;
  onEnter: (raffleId: number, userId: string) => Promise<EnterResult>;
  hasEntered: boolean;
  ...
  return (
    <div className="raffle-card raffle-card-modern">
      ...
      >
        ...
        {/* WINNER BADGE */}
        {isWinner && <div className="raffle-winner-badge">🏆 You Won</div>}
        {/* PRIZE */}
        <div className="raffle-prize-tag">
          <span>{getPrizeIcon(raffle.prize_type)}</span>
          <span className="raffle-prize-value">{raffle.prize_value}</span>
        </div>
      </div>
      ...
      <div className="raffle-card-body">
        ...
        <div className="raffle-meta">
          ...
          <div className="raffle-progress">
            ...
            <div className="raffle-progress-bar">
              <div className="raffle-progress-fill" style={{ width: `${entriesPercent}%` }} />
            </div>
          </div>
        </div>
        ...
        {message && (
          <div
            className={`raffle-message ${
              message.includes('in!') ? 'raffle-message-success' : 'raffle-message-error'
            }`}
          >
            {message}
          </div>
        ...
        {isWinner ? (
          ...
          <button className="raffle-enter-btn" onClick={handleEnter} disabled={entering}>
...
const RafflePage: React.FC = () => {
  ...
  const [enteredMap, setEnteredMap] = useState<Record<number, string>>({});
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (userId) {
          ...
          entriesData.forEach((entry: RaffleEntry) => {
  ...
  /* ENTER */
  const handleEnterRaffle = async (raffleId: number, uid: string): Promise<EnterResult> => {
    try {
  ...
  const hasEntered = (id: number): boolean => !!enteredMap[id];
  const getTicketNum = (id: number): string | undefined => enteredMap[id];
  ...
  return (
    <div className="raffle-page">
      ...
      <div className="raffle-hero">
        <span className="raffle-hero-tag">Limited Time</span>
        <h1 className="raffle-hero-title">
          Win
          <br />
          <em className="hollow">Big.</em>
        </h1>
      ...
      <div className="raffle-container">
        ...
        <div className="raffle-filters">
          {['all', 'live', 'upcoming', 'closed'].map((f) => (
            <button
              key={f}
              className={`raffle-filter-btn ${filter === f ? 'active' : ''}`}
              onClick={() => setFilter(f)}
            >
              {f.charAt(0).toUpperCase() + f.slice(1)}
              <span className="raffle-filter-count">{counts[f as keyof typeof counts]}</span>
            </button>
          ))}
        </div>
            ...
            {Array.from({ length: 4 }).map((_, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  {isEven && <div className="raffle-divider">/</div>}
                  <RaffleCardSkeleton />
                </React.Fragment>
              );
            })}
        ...
        {/* ERROR */}
        {error && <div className="raffle-error">{error}</div>}
