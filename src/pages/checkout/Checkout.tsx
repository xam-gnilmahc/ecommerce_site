...
import { buildPaymentRequest, getUpdatedPaymentData } from '../../components/product/GooglePlay';
...
import { useCartItems } from '../../tanstack/cart';
import { usePlaceOrder } from '../../tanstack/orders';
import { trackPurchase } from '../../tanstack/tracking';
import { processCardPayment, processGooglePay } from '../../services/googlePayService';
...
const Checkout: React.FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'googlePay' | 'card'>('googlePay');
  ...
  const [newAddressStates, setNewAddressStates] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) return;
      setAddresses(data || []);
      const def = data?.find((a: ShippingAddress) => a.is_default) || data?.[0];
      if (def) setSelectedAddressId(def.id);
    })();
  }, [user?.id]);

  const getSubtotal = () => cart.reduce((s: number, i: CartItem) => s + i.amount * i.quantity, 0);
  ...
  const handleNewAddressCountryChange = (c: string) => {
  ...
  const handleOrderSuccess = async (orderId: any) => {
    if (!orderId) return;
    trackPurchase(user?.id, { id: orderId }).catch(() => {});
    setShow(true);
  };

  const handlePaymentError = (result: any): boolean => {
  ...
  useEffect(() => {
    ...
    const displayItems = cart.map((i: CartItem) => ({
                              ...
                              <div className="ck-field">
                                ...
                                <select
                                  ...
                                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                                    handleNewAddressCountryChange(e.target.value)
                                  }
                                  required
                                >
                    ...
                    <div className="ck-gpay-box">
                      {paymentRequest && user ? (
                        <GooglePayButton
                          ...
                          onPaymentDataChanged={(d: any) =>
                            getUpdatedPaymentData(paymentRequest, d) as any
                          }
                        />
                      ) : (
              ...
              <div className="ck-summary">
                <div className="ck-summary-head">
                  ...
                  <span className="ck-summary-count">
                    {cart.reduce((s: number, i: CartItem) => s + i.quantity, 0)} items
                  </span>
                </div>
