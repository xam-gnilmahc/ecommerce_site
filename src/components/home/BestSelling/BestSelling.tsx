...
import { useBestSellingProducts, useRecentlyViewedProducts } from '../../../tanstack/products';
  ...
  const ProductCard: React.FC<ProductCardProps> = ({ product }) => (
    <div className="lpContainer">
      <div className="lpImageContainer">
        <Link to={`/product/${product.id}`} onClick={() => trackProduct(String(product.id))}>
          <img
      ...
      <div className="limitedProductInfo">
        ...
        <div className="productRatingReviews">
          {[...Array(5)].map((_, i) => (
            <FaStar key={i} size={10} color={i < (product.rating || 0) ? '#FEC78A' : '#ddd'} />
          ))}
