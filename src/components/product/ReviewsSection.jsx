...
const ReviewsSection = ({ productId, productName, initialReviews = [], user }) => {
  ...
  useEffect(() => {
    ...
    const merged = [
      ...storedReviews,
      ...initialReviews.filter((r) => !storedReviews.some((sr) => sr.id === r.id)),
    ];
    setReviews(merged);
      ...
      <div className="reviews-body">
        ...
        <div className="rating-card">
          <div className="rating-card-top">
            ...
            <div className="rating-big-stars">
              {getStarArray(parseFloat(avgRating)).full > 0 &&
                [...Array(getStarArray(parseFloat(avgRating)).full)].map((_, i) => (
                  <FaStar key={i} className="filled" />
                ))}
              {getStarArray(parseFloat(avgRating)).half && (
                <FaStarHalfAlt key="half" className="filled" />
              )}
              {getStarArray(parseFloat(avgRating)).empty > 0 &&
                [...Array(getStarArray(parseFloat(avgRating)).empty)].map((_, i) => (
              ...
              return (
                <div key={star} className="bar-row">
                  ...
                  <div className="bar-track">
                    <div className="bar-fill" style={{ width: `${pct}%` }} data-rating={star} />
                  </div>
        ...
        <div className="write-card">
          ...
          <form onSubmit={handleSubmit} className="write-form">
            ...
            <div className="write-form-group">
              <label>
                Add Photos/Videos <span className="optional">(optional, max 5)</span>
              </label>
