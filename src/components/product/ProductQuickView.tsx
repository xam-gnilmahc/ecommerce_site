...
import { trackAddToCart } from '../../tanstack/tracking';
...
const ProductQuickView: React.FC<ProductQuickViewProps> = ({ productId, onClose }) => {
  ...
  const handleAddToCart = async () => {
    ...
    addCartMutation.mutate({ userId: user.id, product: product as Product });
    await trackAddToCart(user?.id, product as Product);
  };
              ...
              <div className="lg:w-1/2 flex flex-col p-6 sm:p-8 lg:p-10">
                ...
                <div className="mt-6">
                  <p className="text-[14px] text-[#555] leading-[1.7] m-0">
                    ...
                    {product.description?.length && product.description.length > 350 ? '...' : ''}
                  </p>
                </div>
