...
const ProductImageGallery: React.FC<ProductImageGalleryProps> = ({
  ...
  const getUrl = (img: string | ImageItem): string => {
    if (!img) return '';
    if (typeof img === 'string') return `${SUPABASE_IMG_BASE}${img}`;
    return `${SUPABASE_IMG_BASE}${img.image_url}`;
  };

  const getImageKey = (img: string | ImageItem, idx: number): string | number => {
    if (!img) return idx;
  ...
  useEffect(() => {
    if (thumbListRef.current) {
      const activeThumb = thumbListRef.current.children[activeIndex] as HTMLElement;
      if (activeThumb) {
